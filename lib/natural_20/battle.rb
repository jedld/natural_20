...
module Natural20
  class Battle
    ...
    def while_active(max_rounds = nil, &block)
      loop do
        Natural20::EventManager.received_event({ source: self, event: :start_of_round, target: current_turn })
        ...
        if @current_turn_index >= @combat_order.length
          ...
          Natural20::EventManager.received_event({ source: self, event: :top_of_the_round, round: @round, target: current_turn })
